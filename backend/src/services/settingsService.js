    ...
    if (!settings) {
      ...
      Object.keys(updates).forEach(key => {
        if (settings.schema.paths[key]) {
          ...
          // For Mixed types (like trainingLabels), explicitly mark as modified
          // This ensures nested changes are detected by Mongoose
          if (key === 'trainingLabels' || key === 'emailConfig') {
