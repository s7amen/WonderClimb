...
const TrainingReport = () => {
    ...
    const fetchReport = async () => {
        try {
            setLoading(true);
            const response = await financeAPI.getTrainingReport(dates.startDate, dates.endDate);
            // Ensure transactions exist even if backend doesn't send them yet, to avoid crashes
            const reportData = response.data.report;
            if (!reportData.transactions) reportData.transactions = [];
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                    <h1 className="text-2xl font-medium text-neutral-950">Отчет: Приходи от тренировки</h1>
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                            ...
                            <span className="text-2xl font-bold text-green-600 block">{formatCurrency(report.totalRevenue)}</span>
                        </div>
                        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                            <span className="text-sm text-gray-500 block mb-1">Разходи (Треньори)</span>
                            <span className="text-2xl font-bold text-red-500 block">{formatCurrency(report.coachFees?.total)}</span>
                        </div>
                        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                            ...
                            <span className="text-xl font-bold text-blue-600 block">{formatCurrency(report.passesSold?.revenue)}</span>
                        </div>
                        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                            <span className="text-sm text-gray-500 block mb-1">От Тренировки</span>
                            <span className="text-xl font-bold text-teal-600 block">{formatCurrency(report.sessionBookings?.revenue)}</span>
                        </div>
                    </div>
