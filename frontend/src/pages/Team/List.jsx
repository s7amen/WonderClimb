...
const TeamList = () => {
    ...
    useEffect(() => {
        fetchUsers();
    }, [page, limit, sortField, sortDirection, filterStatus]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    const fetchUsers = async () => {
        try {
            ...
            const params = {
                ...
                role: 'coach' // Filter only coaches
            };
            ...
            showToast(
                error.response?.data?.error?.message || 'Грешка при зареждане на треньори',
                'error'
            );
        } finally {
            setLoading(false);
    ...
    if (loading) {
        return (
            <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <Loading text="Зареждане на треньори..." />
            </div>
        );
    }
