...
const CompetitionDetail = () => {
  ...
  const fetchCompetition = async () => {
    try {
      setLoading(true);
      const response = await competitionsAPI.getCompetitionAdmin(id);
      if (response.data.competition) {
        setCompetition(response.data.competition);
      } else {
        showToast('Състезанието не е намерено', 'error');
        navigate('/competitions/manage');
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="border border-[rgba(0,0,0,0.1)] rounded-[10px]">
          ...
          <div className="space-y-4">
            ...
            {competition.importedAt && (
              <div>
                <label className="text-sm font-medium text-[#4a5565]">Импортирано на</label>
                <p className="text-sm text-neutral-950 mt-1">
                  {format(new Date(competition.importedAt), 'dd MMMM yyyy, HH:mm')}
                </p>
              </div>
            )}
            <div>
              <label className="text-sm font-medium text-[#4a5565]">Създадено на</label>
              <p className="text-sm text-neutral-950 mt-1">
                {format(new Date(competition.createdAt), 'dd MMMM yyyy, HH:mm')}
              </p>
            </div>
            {competition.updatedAt && competition.updatedAt !== competition.createdAt && (
              <div>
                <label className="text-sm font-medium text-[#4a5565]">Обновено на</label>
                <p className="text-sm text-neutral-950 mt-1">
                  {format(new Date(competition.updatedAt), 'dd MMMM yyyy, HH:mm')}
                </p>
              </div>
            )}
          </div>
