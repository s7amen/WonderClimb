...
const Bookings = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const errorMessage = error.response?.data?.error?.message || 'Грешка при зареждане на резервации';
  ...
  // Group bookings by session (for climbers, typically one booking per session, but we handle multiple)
  ...
  return (
    <div className="space-y-6 px-4 md:px-0">
      <div className="space-y-2">
        <h1 className="text-xl font-medium text-neutral-950 leading-8 mb-1">Моят график</h1>
        <p className="text-base leading-6" style={{ color: '#4a5565' }}>Преглед на резервираните тренировки</p>
      </div>
      ...
      {upcomingBookings.length > 0 && (
        <div className="flex flex-col md:flex-row justify-end items-start md:items-center gap-3 pt-4 border-t-2 border-gray-300">
          ...
          {upcomingBookings.length === 0 ? (
            <Card>
              <div className="px-6 py-12 text-center">
                <p className="text-base font-medium text-neutral-950 mb-2">Няма предстоящи резервации</p>
                <p className="text-sm" style={{ color: '#4a5565' }}>Вашите резервации ще се появят тук</p>
              </div>
            </Card>
          ) : (
            days.map((day) => {
                  ...
                  <div>
                    {dayData.sessions.map(({ session, bookings }, index) => {
                      ...
                      return (
                        <div
                          key={session._id}
                          className="px-4 py-3 bg-white border-b border-gray-100 last:border-b-0"
                        >
                          {/* Mobile Layout */}
